...
var arrayLength = 0;
class FirebaseSvc {
  observeAuth = () => auth().onAuthStateChanged(this.onAuthStateChanged);
  ...
  get ref() {
    return database().ref(
      'Messages/' + global.receiverUid + '/' + auth().currentUser.uid,
    );
  }
  ...
  refOn = async (callback) => {
    database()
      .ref('Messages/' + auth().currentUser.uid + '/' + global.receiverUid)
      .orderByKey()
      .endAt('A')
      .startAt('-')
      .limitToLast(1)
      .on(
        'child_added',
        async (snapshot) => await callback(await this.parse(snapshot)),
      );
  };
  ...
  send = async (messages, p, images, index) => {
    EncryptedStorage.setItem(
      'IsRequest/' + auth().currentUser.uid + '/' + global.receiverUid,
      'false',
    );
    database()
      .ref('Messages/' + auth().currentUser.uid + '/' + global.receiverUid)
    ...
    EncryptedStorage.setItem(
      'ShowMessageBox/' + auth().currentUser.uid + '/' + global.receiverUid,
      'true',
    );
    if (global.firstMessage) {
      var kExists = false;
      var kListener = database().ref(
        'Messages/' + global.receiverUid + '/' + auth().currentUser.uid + '/k',
      );
      ...
      if (!kExists) {
        ...
        var receiverRef = firestore()
          .collection(global.receiverUid)
          .doc('MessageInformation');
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      /*fetch('https://onesignal.com/api/v1/notifications', {
        ...
        body: JSON.stringify({
          app_id: '7af3b2d1-d4fe-418d-a096-4f57f2c384c8',
      ...
      if (p == 't') {
        ...
        await RNFS.mkdir(
          RNFS.DocumentDirectoryPath + '/' + auth().currentUser.uid,
        );
        await RNFS.copyFile(
          images[index].url,
          RNFS.DocumentDirectoryPath +
