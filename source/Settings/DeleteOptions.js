...
var isSelectedArray = [];
var textArray = ['first', 'second', 'third', 'fourth', 'fifth'];
    ...
    if (auth().currentUser.email == global.deleteAuthEmail) {
      ...
      if (authenticated) {
        ...
        messageUidsArray.get().then(async (doc) => {
          ...
          if (doc.exists) {
            ...
            for (let i = 0; i < conversationUidArray.length; i++) {
              EncryptedStorage.removeItem(
                auth().currentUser.uid + conversationUidArray[i] + '/messages',
              );
              EncryptedStorage.removeItem(
                'IsRequest/' +
        ...
        storage()
          ...
          .delete();
        storage()
          .ref('Photos/' + auth().currentUser.uid + '/SearchPhotos/vec.pickle')
          .delete();
      ...
      return (
        <View
          ...
          <CustomHeader
            ...
            title={'Delete Account'}></CustomHeader>
